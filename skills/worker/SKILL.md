...
0.1. **WORKER-OWNED STATE (Claim Protocol)**: Upon activation, immediately move your task file from `./tasks/<task_id>.json` to your workspace at `./tmp/<task_id>/task.json`. This "claims" the task and prevents supervisor interference.
0.2. **EPHEMERALITY**: Once the task status is set to `completed`, you MUST move the final `./tmp/<task_id>/task.json` back to `./tasks/<task_id>.json`, delete your temporary workspace, and terminate your `zmx` session using the `run_shell_command` with `zmx kill <session_name>`.

1. **Task Loading**: Read your claimed task file in `./tmp/<task_id>/task.json`.
...
4. **Task Update & Artifact Verification**:
   - Use the `update_task` tool to set the status to `in_progress`.
   - When finished, use `update_task` with the `completed` status AND a comma-separated list of `produced_artifacts` (paths to logs, reports, or code changes).
   - The system will verify the existence of these artifacts before committing the status change.
